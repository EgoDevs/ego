...
import { getCanisterId, hasOwnProperty, getActor } from '@ego-js/utils';
...
beforeAll(async () => {
  for (let i = 0; i < 2; i += 1) {
    let storeActor = getActor<_SERVICE>(Ed25519KeyIdentity.fromJSON(JSON.stringify(operators[i]?.identity)), idlFactory, getCanisterId('ego_dev')!);
  ...
  for (let i = 0; i < 2; i += 1) {
    let storeActor = getActor<_SERVICE>(Ed25519KeyIdentity.fromJSON(JSON.stringify(developers[i]?.identity)), idlFactory, getCanisterId('ego_dev')!);
  ...
  let storeActor = getActor<_SERVICE>(Ed25519KeyIdentity.fromJSON(JSON.stringify(admins[0]?.identity)), idlFactory, getCanisterId('ego_dev')!);
