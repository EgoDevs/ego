...
describe('ego_store_backup', () => {
  it('backup_data', async () => {
    const actor = await getActor<Service>(identity(), Idl, getCanisterId('ego_store')!);
...
describe('ego_tenant_backup', () => {
  it('backup_data', async () => {
    const actor = await getActor<Service>(identity(), Idl, getCanisterId('ego_tenant')!);
...
describe('ego_dev_backup', () => {
  it('backup_data', async () => {
    const actor = await getActor<Service>(identity(), Idl, getCanisterId('ego_dev')!);
  ...
  for (const job of job_list) {
    ...
    for (let start = 0; start < amount; start += step) {
      ...
      if (!exists) {
        ...
        const response = await actor.job_data_backup(name, BigInt(start), BigInt(end));
