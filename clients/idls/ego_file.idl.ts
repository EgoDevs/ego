export const idlFactory = ({ IDL }) => {
  ...
  const Result_6 = IDL.Variant({ 'Ok' : IDL.Vec(IDL.Nat8), 'Err' : EgoError });
  const Result_7 = IDL.Variant({ 'Ok' : IDL.Bool, 'Err' : EgoError });
  return IDL.Service({
    ...
    'file_main_read' : IDL.Func([IDL.Text], [Result_6], ['query']),
    'file_main_write' : IDL.Func(
        [IDL.Text, IDL.Text, IDL.Vec(IDL.Nat8)],
        [Result_7],
        [],
      ),
    'state_persist' : IDL.Func([], [Result_7], []),
    'state_restore' : IDL.Func([], [Result_7], []),
  });
};
