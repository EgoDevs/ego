export const idlFactory = ({ IDL }) => {
  ...
  const Result_1 = IDL.Variant({ 'Ok' : UserApp, 'Err' : EgoError });
  const Result_2 = IDL.Variant({ 'Ok' : IDL.Vec(UserApp), 'Err' : EgoError });
  const Result_3 = IDL.Variant({ 'Ok' : IDL.Null, 'Err' : EgoError });
  const AdminWalletCycleRechargeRequest = IDL.Record({
    'cycle' : IDL.Nat,
    'comment' : IDL.Text,
    'wallet_id' : IDL.Principal,
  });
  const Result_4 = IDL.Variant({ 'Ok' : IDL.Bool, 'Err' : EgoError });
  ...
  const Result_5 = IDL.Variant({ 'Ok' : IDL.Vec(Order), 'Err' : EgoError });
  ...
  return IDL.Service({
    ...
    'admin_wallet_app_list' : IDL.Func([IDL.Principal], [Result_2], []),
    ...
    'admin_wallet_list' : IDL.Func([], [Result], []),
    'admin_wallet_order_list' : IDL.Func([], [Result_5], []),
