use ic_cdk::api::management_canister::main::{CanisterInstallMode, CanisterSettings, create_canister_with_extra_cycles, CreateCanisterArgument, delete_canister, deposit_cycles, install_code, InstallCodeArgument, stop_canister};
...
pub async fn canister_main_create(cycles_to_use: Cycles) -> Result<Principal, EgoError> {
  ...
  match create_canister_with_extra_cycles(in_arg, cycles_to_use).await {
    ...
    Err((code, msg)) => {
      let code = code as u16;
      error!(error_code = code, error_message = msg.as_str(), "Error calling canister_main_create");
      Err(EgoError { code, msg })
    }
  }
}
...
pub async fn canister_main_delete(canister_id: Principal) -> Result<(), EgoError> {
  ...
  let _delete_result = match delete_canister(CanisterIdRecord{ canister_id }).await {
    ...
    Err((code, msg)) => {
      let code = code as u16;
      error!(error_code = code, error_message = msg.as_str(), "Error calling delete_canister");
      Err(EgoError { code, msg })
    }
  }?;
...
pub async fn canister_cycle_top_up(
  ...
  match deposit_cycles(CanisterIdRecord{canister_id}, cycles_to_use).await {
    ...
    Err((code, msg)) => {
      let code = code as u16;
      error!(error_code = code, error_message = msg.as_str(), "Error calling deposit_cycles");
      Err(EgoError { code, msg })
    }
  }
}
