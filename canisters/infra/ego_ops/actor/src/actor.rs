...
use ego_ops_mod::types::{
    AdminAppCreateRequest, AdminWalletCycleRechargeRequest, AdminWalletProviderAddRequest,
};
    ...
    match name.as_str() {
        "ego_dev" => {
            for ego_file_id in ego_file_ids.iter() {
                ego_canister.ego_canister_add(
                    ego_dev_id,
                    "ego_file".to_string(),
                    ego_file_id.clone(),
                );
            }
            ego_canister.ego_canister_add(ego_dev_id, "ego_store".to_string(), ego_store_id);
            ego_canister.ego_canister_add(
                ego_dev_id,
                "ego_tenant".to_string(),
                ego_tenant_id.clone(),
            );
            ego_canister.ego_canister_add(
                ego_dev_id,
                "ego_record".to_string(),
                ego_record_id.clone(),
            );
        }
        "ego_file" => {
...
pub fn admin_app_create(req: AdminAppCreateRequest) -> Result<(), EgoError> {
...
pub fn admin_wallet_provider_add(req: AdminWalletProviderAddRequest) -> Result<(), EgoError> {
    ...
    ego_store.admin_wallet_provider_add(req.wallet_provider, req.wallet_app_id);
...
pub fn admin_wallet_cycle_recharge(req: AdminWalletCycleRechargeRequest) -> Result<(), EgoError> {
    info_log_add("ego_ops: admin_wallet_cycle_recharge");

    let ego_store_id = canister_get_one("ego_store").unwrap();
    let ego_store = EgoStore::new(ego_store_id);

    ego_store.admin_wallet_cycle_recharge(req.wallet_id, req.cycle, req.comment);
...
pub fn admin_wallet_order_new(amount: f32) -> Result<(), EgoError> {
    info_log_add("ego_ops: admin_wallet_order_new");

    let ego_store_id = canister_get_one("ego_store").unwrap();
    let ego_store = EgoStore::new(ego_store_id);

    ego_store.admin_wallet_order_new(amount);
