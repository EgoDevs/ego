...
pub fn set_up() {
    ...
    EGO_STORE.with(|ego_store| {
        ...
        let ego_store_app = EgoStoreApp::new(app.clone(), wasm);
        ...
        let test_ego_store_app = EgoStoreApp::new(test_app.clone(), test_wasm);
        ego_store
            .borrow_mut()
            .apps
        ...
        let new_wallet = Wallet::new(tenant_principal, wallet_principal, user_principal);
        ego_store
            .borrow_mut()
            .wallets
...
fn admin_wallet_app_transfer_no_wallet() {
    ...
    assert!(result.is_err());
    assert_eq!(
        "wallet and app can not both be null",
...
fn admin_wallet_app_transfer_wrong_canister() {
    ...
    assert!(result.is_err());
    assert_eq!(
        format!("canister {} not exists", backend_principal),
        result.err().unwrap().msg
    );
}
