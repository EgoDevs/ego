...
fn test_export() {
  ...
  let result = record_export("ego_store_apps".to_string(), None).expect("record not founded");
  ...
  let result = record_export("tenants".to_string(), None).expect("record not founded");
  ...
  let result = record_export("wallet_providers".to_string(), None).expect("record not founded");
  ...
  let result = record_export("wallets".to_string(), None).expect("record not founded");
  ...
  let result = record_export("user_apps".to_string(), None).expect("record not founded");
  ...
  let result = record_export("orders".to_string(), None).expect("record not founded");
  ...
  let result = record_export("cash_flows".to_string(), None).expect("record not founded");
...
fn test_export_with_last_update() {
  ...
  let result = record_export("ego_store_apps".to_string(), last_update).expect("record not founded");
  assert_eq!("ego_store_apps", result.name);
  let ego_store_apps: Vec<EgoStoreApp> = serde_json::from_slice(&result.data).unwrap();
  assert_eq!(0, ego_store_apps.len());

  let result = record_export("tenants".to_string(), last_update).expect("record not founded");
  assert_eq!("tenants", result.name);
  let tenants: Vec<Tenant> = serde_json::from_slice(&result.data).unwrap();
  assert_eq!(1, tenants.len());
  assert_eq!(TENANT_ID1, tenants.get(0).unwrap().canister_id.to_string());

  let result = record_export("wallet_providers".to_string(), last_update).expect("record not founded");
  assert_eq!("wallet_providers", result.name);
  let wallet_providers: Vec<WalletProvider> = serde_json::from_slice(&result.data).unwrap();
  assert_eq!(1, wallet_providers.len());
  assert_eq!(WALLET_PROVIDER_ID, wallet_providers.get(0).unwrap().wallet_provider.to_string());

  let result = record_export("wallets".to_string(), last_update).expect("record not founded");
  assert_eq!("wallets", result.name);
  let wallets: Vec<Wallet> = serde_json::from_slice(&result.data).unwrap();
  assert_eq!(0, wallets.len());

  let result = record_export("user_apps".to_string(), last_update).expect("record not founded");
  assert_eq!("user_apps", result.name);
  let user_apps: Vec<UserApp> = serde_json::from_slice(&result.data).unwrap();
  assert_eq!(0, user_apps.len());

  let result = record_export("orders".to_string(), last_update).expect("record not founded");
  assert_eq!("orders", result.name);
  let orders: Vec<Order> = serde_json::from_slice(&result.data).unwrap();
  assert_eq!(0, orders.len());

  let result = record_export("cash_flows".to_string(), last_update).expect("record not founded");
  assert_eq!("cash_flows", result.name);
  let cash_flows: Vec<CashFlow> = serde_json::from_slice(&result.data).unwrap();
