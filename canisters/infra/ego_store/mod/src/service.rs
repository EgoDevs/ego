...
use crate::state::{info_log_add, EGO_STORE};
...
impl EgoStoreService {
    ...
    pub fn wallet_cycle_list(wallet_id: Principal) -> Result<Vec<CashFlow>, EgoError> {
        let cash_flows =
            EGO_STORE.with(|ego_store| ego_store.borrow().wallet_cycle_list(&wallet_id))?;
        Ok(cash_flows)
    }

    pub fn wallet_cycle_balance(wallet_id: Principal) -> Result<u128, EgoError> {
        let balance =
            EGO_STORE.with(|ego_store| ego_store.borrow().wallet_cycle_balance(&wallet_id))?;
        Ok(balance)
    }
    ...
    pub fn wallet_order_notify(memo: Memo, operator: Principal, ts: u64) -> Result<bool, EgoError> {
        EGO_STORE.with(|ego_store| {
            ego_store
                .borrow_mut()
                .wallet_order_notify(memo, operator, ts)
        })
    }
    ...
    pub fn admin_wallet_provider_add(wallet_provider: &Principal, wallet_id: &AppId) {
        EGO_STORE.with(|ego_store| {
            ego_store
                .borrow_mut()
    ...
    pub fn admin_wallet_app_transfer(
        ...
        if let Some(wallet_id) = wallet {
            ...
            let user_app =
                EGO_STORE.with(
                    |ego_store| match ego_store.borrow().wallets.get(&wallet_id) {
                        None => Err(EgoError::from(format!("wallet {} not exists", wallet_id))),
                        Some(wallet) => match wallet.apps.get(&canister_id) {
                            None => Err(EgoError::from(format!(
                                "canister {} not exists",
                                canister_id
                            ))),
            ...
            if let Some(app_id) = app {
                let ego_store_app =
                    EGO_STORE.with(|ego_store| match ego_store.borrow().apps.get(&app_id) {
                        None => Err(EgoError::from(format!("app {} not exists", app_id))),
                        Some(ego_store_app) => Ok(ego_store_app.clone()),
                    })?;
                ...
                return Err(EgoError::from(
                    "wallet and app can not both be null".to_string(),
                ));
            }
        };
