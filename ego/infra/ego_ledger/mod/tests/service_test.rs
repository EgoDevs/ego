...
use ic_ledger_types::{
  AccountIdentifier, Block, BlockIndex, Memo, Subaccount, Timestamp, Tokens, Transaction,
};
...
fn ledger_main_init() {
  ...
  EGO_LEDGER.with(|ego_ledger| assert_eq!(1, ego_ledger.borrow().start));
}
...
async fn ledger_payment_match_ic_ledger_error() {
  ...
  ic_ledger
    .expect_query_blocks()
    .returning(|_block| Err(EgoError::from("ic ledger error".to_string())));
