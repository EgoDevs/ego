...
impl EgoLedger {
  ...
  pub fn ledger_payment_add(&mut self, payment: Payment) {
    self.wait_for_notified.entry(payment.memo).or_insert(payment);
  }

  pub fn ledger_payment_matched(&self) -> Vec<Memo> {
    let memos = self.wait_for_notified.iter().filter_map(|(w_memo, w_payment)| {
      match self.confirmed.get(w_memo) {
        Some(c_payment) => {
