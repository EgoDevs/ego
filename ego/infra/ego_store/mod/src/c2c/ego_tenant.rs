...
use ego_tenant_mod::types::{AppMainInstallRequest, AppMainInstallResponse, AppMainUpgradeRequest, AppMainUpgradeResponse, CanisterMainTrackRequest, CanisterMainUnTrackRequest};
...
impl TEgoTenant for EgoTenant {
  async fn app_main_install(&self, ego_tenant_id: Principal, wallet_id: Principal, user_id: Principal, wasm: Wasm) -> Result<Principal, EgoError> {
    let req = AppMainInstallRequest { wallet_id, user_id, wasm };
  ...
  async fn canister_main_track(&self, ego_tenant_id: Principal, wallet_id: Principal, canister_id: Principal) -> Result<(), EgoError> {
    ...
    let call_result = api::call::call(
      ...
      .await as Result<(Result<(), EgoError>, ), _>;
