...
impl EgoTenant {
  pub fn new() -> Self {
    EgoTenant {
      ...
      ego_store: Principal::anonymous()
    }
  }

  pub fn canister_main_track(&mut self, wallet_id: Principal, canister_id: Principal) -> Result<(), EgoError> {
    self.tasks.entry(canister_id).or_insert(Task::new(wallet_id, canister_id));
    Ok(())
  }

  pub fn canister_main_untrack(&mut self, _wallet_id: Principal, canister_id: Principal) -> Result<(), EgoError> {
