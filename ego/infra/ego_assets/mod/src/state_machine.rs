...
pub struct StableState {
    ...
    pub stable_assets: HashMap<String, Asset>,
}
...
impl State {
    ...
    pub fn list_authorized(&self, caller: &Principal) -> Result<Vec<Principal>, String> {
        if !self.is_authorized(caller) {
            return Err("the caller is not authorized".to_string());
        }
    ...
    pub fn drain(&mut self, caller: &Principal) -> Result<(), String> {
        let this_caller = caller.clone();
        if !self.is_authorized(&this_caller.clone()) {
            return Err("the caller is not authorized".to_string());
        }
        self.authorized.retain(|x| x.eq(&this_caller.clone()));
        Ok(())
    }
    ...
    pub fn store(&mut self, arg: StoreArg, time: u64) -> Result<(), String> {
        ...
        let hash = sha2::Sha256::digest(&(arg.content).as_ref()).into();
