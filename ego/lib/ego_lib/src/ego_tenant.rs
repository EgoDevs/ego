...
impl TEgoTenant for EgoTenant {
  fn ego_cycle_check_cb(&self, records: Vec<CycleRecord>, threshold: u128) {
    let _result = api::call::notify(self.canister_id, "ego_cycle_check_cb", (records, threshold, ));
  }

  async fn wallet_cycle_recharge(&self, cycles: u128) -> Result<(), EgoError>{
    let call_result = api::call::call(self.canister_id, "wallet_cycle_recharge", (cycles, )).await as Result<(Result<(), EgoError>, ), (RejectionCode, String)>;
